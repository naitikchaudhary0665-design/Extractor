...
if uploaded_files:
    if st.button("🚀 Process & Generate Full Excel"):
        try:
            # Automatically find a working flash model available for your API key
            available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
            
            # Pick a flash model or the first available one
            selected_model_name = next((m for m in available_models if 'flash' in m), available_models[0])
            # Remove 'models/' prefix if present because GenerativeModel expects it clean or with prefix depending on version
